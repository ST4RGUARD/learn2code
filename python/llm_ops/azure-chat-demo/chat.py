...
async def main():
    ...
    AZURE_OPEN_AI__CHAT_COMPLETION_DEPLOYMENT_NAME = os.environ[
    ...
    context.variables["user_input"] = (
        "What is the average temperature in Seattle in June?"
    )
    ...
    chat_function._chat_prompt_template.messages.append(
        {"role": "assistant", "content": "It is 85 degrees in Seattle in June"}
    )
    context = await chat_function.invoke_async(context=context, functions=functions)
